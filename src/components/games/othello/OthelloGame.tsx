...
import { ChevronLeft, Circle, Eye, EyeOff } from "lucide-react";
        ...
        {!game || game.status === "waiting" || localLobbyView ? (
          ...
          >
            ...
            <div className="flex flex-col gap-3">
              ...
              />
              <div className="flex items-center justify-between px-2 text-sm font-black">
                <div className="flex items-center gap-2 rounded-full border border-gray-100 bg-white px-3 py-1.5 shadow-sm">
                  <div className="h-3 w-3 rounded-full bg-sky-400 shadow-sm" />
                  <span className="text-gray-700">
                    하늘색: {currentDiscs.black}
                  </span>
                </div>

                <button
                  onClick={() => setShowGuide(!showGuide)}
                  className="flex items-center gap-1.5 rounded-full border border-gray-100 bg-white px-3 py-1.5 text-xs font-bold text-gray-500 shadow-sm transition-colors hover:text-gray-800"
                >
                  {showGuide ? <Eye size={14} /> : <EyeOff size={14} />}
                  <span>힌트 {showGuide ? "ON" : "OFF"}</span>
                </button>

                <div className="flex items-center gap-2 rounded-full border border-gray-100 bg-white px-3 py-1.5 shadow-sm">
                  <div className="h-3 w-3 rounded-full bg-rose-400 shadow-sm" />
                  <span className="text-gray-700">
                    분홍색: {currentDiscs.white}
                  </span>
                </div>
              </div>
            </div>
