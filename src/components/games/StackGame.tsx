...
export default function StackGame({ onBack }: StackGameProps) {
  ...
  const update = useCallback((time: number) => {
    ...
    if (movingBlockRef.current && !gameOverRef.current) {
      const moving = movingBlockRef.current;
      moving.x += speedRef.current * directionRef.current;
      if (moving.x > CANVAS_WIDTH - moving.width / 2 + 50) directionRef.current = -1;
      if (moving.x < -50 + moving.width / 2) directionRef.current = 1;
    }
    ...
    cameraYRef.current += (targetCameraYRef.current - cameraYRef.current) * CAMERA_LERP;
