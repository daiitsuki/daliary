...
export default function Game2048({ onBack }: Game2048Props) {
  ...
  const handleHintUse = () => {
    if (hintUsed) return;
    if (confirm("랜덤한 위치에 1024 타일을 하나 생성할까요? (게임당 1회)")) {
      setTiles((prev) => {
            ...
            >
              <div className="grid grid-cols-4 grid-rows-4 gap-2.5 w-full h-full">
                {Array.from({ length: 16 }).map((_, i) => (
                  <div
                    key={i}
                    className="bg-white/40 rounded-[22px] border border-white/20"
                  />
                ))}
              </div>
              <div className="absolute inset-3 pointer-events-none">
                <AnimatePresence>
                  {tiles.map((tile) => (
                    ...
                    >
                      <div
                        className={`w-full h-full rounded-[20px] md:rounded-[24px] flex items-center justify-center font-black text-xl md:text-2xl border-2 border-white/50 shadow-sm ${
                          tile.value <= 4
                              ...
                              : tile.value <= 64
                                ...
                                : tile.value <= 256
                                  ...
                                  : tile.value <= 1024
                                    ...
                                    : "bg-amber-400 text-white shadow-xl shadow-amber-200"
                        }`}
                      >
                        {tile.value}
                      </div>
              ...
              <AnimatePresence>
                {gameOver && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="absolute inset-0 z-20 bg-rose-900/40 backdrop-blur-md rounded-[32px] flex items-center justify-center p-6 text-center"
                  >
                    <div className="bg-white rounded-[40px] p-8 md:p-10 shadow-2xl w-full max-w-[280px] border border-white/50">
                      ...
                      {(reached2048 || tiles.some((t) => t.value >= 2048)) && (
                        <p className="text-amber-500 text-[10px] font-black mb-6 flex items-center justify-center gap-1">
                          <Star size={10} fill="currentColor" /> 2048 달성 기록
                          있음
                        </p>
                      )}
                      <div className="h-4" />
          ...
          <div className="w-full lg:w-[380px] shrink-0 lg:h-full flex flex-col gap-6">
            ...
            <div className="bg-gray-50/50 rounded-[32px] p-8 border border-gray-100">
              ...
              <ul className="space-y-3">
                ...
                <li className="text-[11px] text-gray-400 font-medium leading-relaxed flex gap-2">
                  <span className="w-1 h-1 bg-gray-300 rounded-full mt-1.5 shrink-0" />
                  매일 오전 12시 기준으로 포인트 제공 내역이 초기화됩니다.
                </li>
              </ul>
            </div>
          </div>
