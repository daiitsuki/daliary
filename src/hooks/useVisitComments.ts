...
export const useVisitComments = (visitId: string | undefined) => {
  ...
  const addCommentMutation = useMutation({
    ...
    onSuccess: (newComment) => {
      ...
      queryClient.setQueryData(['visit_comments', visitId], (old: any) => [...(old || []), newComment]);
              ...
              if (v.id === visitId) {
                ...
                return {
                  ...
                  preview_comments: updatedComments.slice(-2).map((c: any) => ({
                    id: c.id,
                    content: c.content,
                    writer: { nickname: c.writer?.nickname || profile?.nickname }
                  }))
                };
              }
              return v;
  ...
  return { 
    comments, 
    loading: loading || addCommentMutation.isPending || deleteCommentMutation.isPending, 
    addComment, 
    deleteComment, 
