...
const VAPID_PUBLIC_KEY =
  "BHXdwwDFB6xCRvcKwTU0Tx042GdgS1aljUdZC1ECuzH4EEu1fT2ChM72Yrt82hVTGmFxVvDNLJT5-Yw7H2eHcS4";
...
export const useNotifications = (userId: string | null) => {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    if (data) {
      ...
      const isActuallyEnabled =
        data.is_enabled && isPermissionGranted && hasActiveSubscription;
  ...
  const updateGranularSetting = async (
  ...
  const registerPushSubscription = async () => {
    if (
      ...
      console.log("[Push] Subscription failed: Pre-conditions not met", {
        userId,
