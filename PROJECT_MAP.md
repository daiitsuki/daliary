...
- **`games/`**: 미니 게임 구현체 (2048, 스택 게임, 수박 게임, 블라인드 타이머, 벽돌깨기 등)
...
- `CoupleContext.tsx`, `CouplePointsContext.tsx`: 커플 정보 및 공동 포인트 관리
- `HomeContext.tsx`, `PlacesContext.tsx`, `SchedulesContext.tsx`, `TimetableContext.tsx`: 도메인별 데이터 로딩 상태 공유
- `NotificationsContext.tsx`: 알림 권한 및 설정 정보 관리
- `ConfirmContext.tsx`, `ToastContext.tsx`: 글로벌 알림(토스트 메세지) 및 확인 모달

### 🪝 `src/hooks/` (데이터 패칭 및 훅)
Supabase DB와 통신하거나 복잡한 상태를 관리하는 커스텀 훅들입니다. 기능별로 모듈화되어 있습니다.
- **`core/`**: 핵심 공통 훅 (`useAttendance.ts`, `useCouple.ts`, `useCouplePoints.ts`, `useHomeData.ts`, `useUserStats.ts`)
- **`features/`**: 주요 기능 훅 (`useAnniversaries.ts`, `useHolidays.ts`, `useMemoryFeed.ts`, `useQuestionHistory.ts`, `useSchedules.ts`, `useTimetable.ts`)
- **`games/`**: 게임 관련 로직 훅 (`useConnectFour.ts`, `useGameScore.ts`, `useMultiplayerGame.ts`, `useSwipe.ts`)
- **`map/`**: 지도 관련 로직 훅 (`usePlaceSearch.ts`, `useTrips.ts`, `useVisitComments.ts`, `useVisitVerification.ts`)
- **`system/`**: 시스템 관련 훅 (`useAppUpdate.ts`, `useNotifications.ts`)

### ⚙️ `src/lib/`, `src/utils/`, `src/constants/`, `src/types/`, `src/data/`, `src/assets/` (유틸리티 및 리소스)
- **`lib/`**: `supabase.ts` (DB 연결), `address.ts` (주소 텍스트 처리), `utils.ts`
- **`utils/`**: `imageUtils.ts`, `shareUtils.ts`, `tripHelpers.ts`
- **`constants/`**: `regions.ts` (지역 코드 등 고정 상수)
- **`types/`**: `index.ts`, `external.d.ts`, `kakao.d.ts` (TypeScript 타입 정의)
- **`data/`**: `changelog.ts`, `drawingQuestions.ts`, `holidays.json` (앱 정적 데이터)
- **`assets/`**: 이미지 리소스 (수박 게임 에셋 등)
...
- `seed_questions.sql`: 기본 데이터베이스 질문 시드
- `config.toml`: 로컬 개발용 Supabase 설정
...
- `AGENTS.md`: AI 행동 지침(Karpathy behavioral guidelines)
