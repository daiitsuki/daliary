-- Update handle_notification_trigger to route to drawing answers
...
BEGIN
    ...
    -- CASE 1: Answers (-> notify_communication)
    IF (TG_TABLE_NAME = 'answers') THEN
        IF v_settings IS NULL OR v_settings.notify_communication THEN
    ...
    -- CASE 1.5: Drawing Answers (-> notify_communication)
    ELSIF (TG_TABLE_NAME = 'drawing_answers') THEN
        IF v_settings IS NULL OR v_settings.notify_communication THEN
            v_type := 'drawing_answered'; 
            v_title := '오늘의 드로잉 질문';
            v_content := v_my_nickname || '님이 오늘의 드로잉 질문에 답변했어요!'; 
            v_url := '/home';
            v_should_notify := TRUE;
        END IF;

    -- CASE 2: Schedules (-> notify_schedule_trip)
    ELSIF (TG_TABLE_NAME = 'schedules') THEN
        IF v_settings IS NULL OR v_settings.notify_schedule_trip THEN
    ...
    -- CASE 3: Places (-> notify_schedule_trip)
    ELSIF (TG_TABLE_NAME = 'places') THEN
        IF NEW.status = 'wishlist' AND (v_settings IS NULL OR v_settings.notify_schedule_trip) THEN
            v_type := 'place_added'; 
            v_title := '새로운 장소 발견';
    ...
    -- CASE 4: Visits (-> notify_visit_verified)
    ELSIF (TG_TABLE_NAME = 'visits') THEN
        IF v_settings IS NULL OR v_settings.notify_visit_verified THEN
            ...
            v_url := '/places?tab=memory&visitId=' || NEW.id;
            v_should_notify := TRUE;
        END IF;

    -- CASE 5: Point History (Level up, Items, Game Rewards) (-> notify_game_activity)
    ELSIF (TG_TABLE_NAME = 'point_history') THEN
        IF NEW.type = 'level_up' AND (v_settings IS NULL OR v_settings.notify_game_activity) THEN
            v_type := 'level_up'; 
            v_title := '레벨 업! 🎉'; 
            v_content := '커플 레벨이 올랐어요! 축하합니다!'; 
            v_url := '/home';
            v_should_notify := TRUE;
        ELSIF NEW.type LIKE 'purchase_%' AND (v_settings IS NULL OR v_settings.notify_game_activity) THEN
            v_type := 'item_purchased'; 
            v_title := '아이템 구매';
            v_content := v_my_nickname || '님이 ' || REPLACE(NEW.description, ' 구매', '') || '을 구매하였습니다.'; 
            v_url := '/home';
            v_should_notify := TRUE;
        ELSIF NEW.type LIKE 'game_reward_%' AND (v_settings IS NULL OR v_settings.notify_game_activity) THEN
            v_type := 'game_reward'; 
            v_title := '게임 미션 달성';
    ...
    -- CASE 6: Trips (-> notify_schedule_trip)
...
-- Attach the trigger to drawing_answers
DROP TRIGGER IF EXISTS on_drawing_answers_notification ON public.drawing_answers;
CREATE TRIGGER on_drawing_answers_notification
  AFTER INSERT ON public.drawing_answers
  FOR EACH ROW EXECUTE FUNCTION public.handle_notification_trigger();
