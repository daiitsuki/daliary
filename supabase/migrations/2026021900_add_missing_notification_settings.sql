-- 1. Add missing notification settings columns
ALTER TABLE public.notification_settings 
ADD COLUMN IF NOT EXISTS notify_trip_change BOOLEAN DEFAULT TRUE,
ADD COLUMN IF NOT EXISTS notify_visit_comment BOOLEAN DEFAULT TRUE;

-- 2. Update existing settings to have defaults
UPDATE public.notification_settings 
SET 
    notify_trip_change = TRUE,
    notify_visit_comment = TRUE
WHERE notify_trip_change IS NULL;

-- 3. Update handle_notification_trigger to:
--    a) Handle visit_comments
--    b) Respect notification_settings
...
BEGIN
    ...
    -- If settings don't exist yet, assume TRUE (or handle gracefully)
    IF v_settings IS NULL THEN
        v_should_notify := TRUE; -- Default to true if no settings found
    ELSE
    ...
    ELSIF (TG_TABLE_NAME = 'point_history' AND NEW.type = 'level_up') THEN
        IF v_settings.notify_level_up THEN
            ...
            -- Note: For level up, we also notify the sender (auth.uid()), 
            -- but settings check here is only for partner. 
            -- Sender notification is skipped in this logic block for simplicity 
            -- or handled separately if we want to check sender's settings too.
            -- For now, we preserve the existing partner logic.
        END IF;
    ...
    -- CASE 7: Visit Comments (NEW)
    ELSIF (TG_TABLE_NAME = 'visit_comments') THEN
        IF v_settings.notify_visit_comment THEN
            v_type := 'visit_comment';
            v_title := '새로운 댓글';
            v_content := v_my_nickname || '님이 방문 기록에 댓글을 남겼어요.';
            v_should_notify := TRUE;
        END IF;
    ...
    IF v_should_notify THEN
        ...
        -- Special Case: Level Up (Notify Sender too)
        IF v_type = 'level_up' THEN
            -- Ideally check sender's settings too, but keeping it simple as it's a celebration
            INSERT INTO public.notifications (user_id, couple_id, type, title, content)
...
-- 4. Add Trigger for visit_comments
...
-- 5. Update get_app_init_data to include new settings
...
BEGIN
  ...
  -- 3. Notification Settings (Updated columns)
  SELECT 
    is_enabled, 
