-- Update trigger_level_up_notification to use notify_game_activity
...
BEGIN
  ...
  FOR v_user_record IN (
    ...
    -- Fetch each user's settings (changed notify_level_up to notify_game_activity)
    SELECT is_enabled, notify_game_activity INTO v_settings 
    FROM public.notification_settings 
    WHERE user_id = v_user_record.id;

    -- Send notification if enabled (default to true if settings row missing)
    IF v_settings IS NULL OR (v_settings.is_enabled AND v_settings.notify_game_activity) THEN
...
-- Update handle_notification_trigger to use correct settings
