...
- **`trips` & `trip_plans`**: 여행 계획 및 세부 경로.
- **`game_scores`**: 게임별 최고 점수 및 일일 보상 수령 기록. `(user_id, game_type)` 복합 PK.
- **`game_sessions`**: 블라인드 타이머 등 세션 기반 게임의 검증용 데이터.
- **`couple_items`**: 커플이 보유한 아이템 정보. `(couple_id, item_type)` 복합 PK.

### Notification System Tables

- **`notifications`**: 알림 이력. `is_read` 상태 관리.
- **`notification_settings`**: 사용자별 알림 수신 여부 (세분화된 필드 포함).
- **`push_subscriptions`**: 웹 푸시 구독 정보 (VAPID). `(user_id, endpoint)` 복합 PK로 다중 기기 지원.
...
- **Rewards** (Rebalanced 2026.02.23):
  ...
  - Bad (≤ 1.00s): 50 PT
- **Verification**:
  - **Server-side Timing**: `started_at`과 `stopped_at`의 차이를 계산하여 서버 타임과 클라이언트 보고
    타임(`diff + target`)을 대조.
